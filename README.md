...
## 사용자의 의견 — 실용적 최적 워크플로우

*이 섹션은 실험을 설계하고 진행한 사용자가 직접 작성한 의견입니다.*

### 비용 관점: "비싼 장비보다 비싼 모델이 더 쌀 수 있다"

Local LLM의 속도와 완전성을 확보하려면 비교적 고가의 장비 구매와 상당한 시행착오가 필요하다. 이 시간 및 초기 비용을 고려하면, 처음부터 더 비싼 프론티어 모델을 쓰는 것이 총비용 기준으로 더 저렴할 수 있다.

### 자율성 관점: pi vs Claude Code의 흐름 제어

Claude Code는 위험한 작업에 YES/NO 확인을 요구하며 진행 흐름을 끊는 경우가 흔하다. `allow` 상태로 미리 설정해두면 이 부담은 없어지지만, 예상치 못한 위험성 때문에 모든 권한을 한 번에 맡기기가 부담스럽다.

반면 pi는 기본적으로 모든 권한을 가진 채 동작한다. 그러나 **스텁을 채우는 범위만 맡긴 상태라면** 크게 잘못될 여지가 적다 — 위험성과 자율성의 균형이 이 워크플로우에서 pi가 적합한 이유 중 하나다.

### 속도 관점: 병렬 실행으로 개발 속도 손실 최소화

KV cache 용량이 기본 동작 수준만 확보된다면 **A3B 35B Q4는 CPU 서버에서조차 프론티어 모델의 약 60% 속도**를 낸다.

더 중요한 점은 구조적 병렬성이다:

```
[Claude]  다음 모듈 설계 · 플랜 논의 · 스텁 + pytest 작성
[pi]      이전 스텁 구현 채우기 (동시 진행)
```

다음 모듈을 프론티어 모델과 설계하는 동안 로컬 LLM이 이전 스텁을 채우고 있다면, TDD 방식이 SDD 대비 개발 속도에서 크게 뒤지지 않는다.

### 안정성 관점: 컨텍스트 폭발 방지

pi agent가 컨텍스트 폭발로 죽는 상황은 **처리 단위를 최소화**함으로써 극복 가능하다:

- 한 번에 `README.md` 1개 + Python 스텁 파일 1개만 처리하도록 제한
- 모듈 단위가 이 범위 내에 완결되도록 설계(모듈형 설계 필수)

이 제약이 오히려 모듈 경계를 명확히 강제하는 설계 압력으로 작용한다.
